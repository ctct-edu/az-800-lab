...
1. 仮想ネットワーク画面で、 **[ + 作成 ]** をクリックします。

    <img src="./media/AZ-800_Lab8_03.png" alt="AZ-800_Lab8_03" style="zoom:80%;" />

1.   **[基本]** ページで、次の情報を入力します。指示がないものは規定値のままで構いません。

    | 設定               | 値                                                 |
    | ------------------ | -------------------------------------------------- |
    | サブスクリプション | **あらかじめ入力されているものを使用してください** |
    | リソースグループ   | **AZ80x-RG**                                       |
    | 仮想ネットワーク名 | 一意の名前を指定してください (例 : myVnet)         |
    | 地域 (リージョン)  | **米国東部 (East US)**                             |

    

1.   **[確認と作成]**  ボタンをクリックし、検証に合格することを確認します。確認出来たら、 **[作成]** を押してリソースをデプロイします。
...
1.  **az800l08 -vnet0** 仮想ネットワーク ページの左ペインにある **[設定]** セクションの一覧から、 **[ピアリング]** を選択し、 **[ + 追加]** をクリックします。
    ...
    9.   **[ピアリングの追加]**  ページで次の設定を指定し、 **[追加]** をクリックします。指示がないものは規定値のままで構いません。
